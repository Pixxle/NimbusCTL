use crate::aws::types::{ResourceTag, ServiceType, TaggedResource};
use crate::utils::error::Result;
use crate::utils::helpers::{extract_region_from_arn, extract_resource_id};
...
impl TaggingService {
    ...
    pub async fn tag_resource(&self, resource_arn: &str, tags: &[ResourceTag]) -> Result<()> {
        // For Phase 1, just log - will implement in Phase 2
        tracing::info!("Tagging resource: {}", resource_arn);
        Ok(())
    }

    pub async fn untag_resource(&self, resource_arn: &str, tag_keys: &[String]) -> Result<()> {
        // For Phase 1, just log - will implement in Phase 2
